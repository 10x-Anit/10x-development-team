...
export function listProjects(): unknown[] {
  const db = getDb();
  return db.prepare('SELECT id, name, scope, type, status, path, last_opened_at FROM projects ORDER BY last_opened_at DESC').all();
}
...
export function touchProject(path: string): void {
  const db = getDb();
  db.prepare('UPDATE projects SET last_opened_at = datetime(\'now\') WHERE path = ?').run(path);
}
...
export function createSession(projectId: string, summary?: string): string {
  const db = getDb();
  const id = genId();
  db.prepare('INSERT INTO sessions (id, project_id, summary) VALUES (?, ?, ?)').run(id, projectId, summary || null);
  return id;
}
...
export function getRecentSessions(projectId: string, limit = 5): unknown[] {
  const db = getDb();
  return db.prepare('SELECT * FROM sessions WHERE project_id = ? ORDER BY started_at DESC LIMIT ?').all(projectId, limit);
}
...
export function saveMemory(projectId: string, category: string, content: string): string {
  const db = getDb();
  const id = genId();
  db.prepare('INSERT INTO memories (id, project_id, category, content) VALUES (?, ?, ?, ?)').run(id, projectId, category, content);
  return id;
}

export function getMemories(projectId: string, limit = 20): unknown[] {
  const db = getDb();
  return db.prepare('SELECT * FROM memories WHERE project_id = ? ORDER BY created_at DESC LIMIT ?').all(projectId, limit);
}
