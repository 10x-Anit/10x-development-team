...
For `production`, the plan MUST include:
- architecture review / technical spec
- explicit phase gates
- testing strategy (unit, integration, e2e)
- API contract verification
- migration safety review if data changes are expected
- performance budget
...
## Step 3: Architecture Review and Technical Spec

For `production`, and for MVPs with auth/payments/database changes, create `.10x/specs/[feature-or-release].md` before implementation starts.

The spec must include:
- summary of the user-visible outcome
- architecture and data flow
- component ownership by agent
- API contracts and shared types/schemas
- database and migration plan
- security threat model
- test pyramid plan
- performance budgets
- deployment, smoke test, and rollback plan

Do not delegate implementation until the spec is complete and the acceptance criteria are explicit.
...
For `production`, create gate tasks at minimum:
- `spec-approved`
- `design-approved`
...
2. **Architecture / contract review** (team-lead owns; backend-dev and frontend-dev consult for production)
3. **Design system** → ui-designer
4. **3D experiences** → 3d-designer (if `vision.three_d` or `vision.immersive` or `vision.scroll_animations` is true — install R3F/GSAP deps, build scenes, particles, scroll effects)
5. **Backend APIs + contract types** → backend-dev (if scope requires)
6. **Frontend pages / consumers** → frontend-dev (imports shared contracts and 3D components when present)
7. **Tests by layer** → qa-tester (if scope requires)
8. **Deployment / release plan** → deployer (if scope requires)

Every delegation for `production` must include the current gate, definition of done, and required handoff fields:
- files changed
- contracts changed
...
For `production`, stop at each gate and verify:

### `spec-approved`
- spec exists and covers architecture, contracts, testing, security, rollout, rollback

### `design-approved`
- design tokens and reusable components are defined before page implementation

### `implementation-complete`
- planned code is merged into the working tree
- API contracts and frontend consumers are aligned
- migration notes exist for any schema change

### `qa-passed`
- unit, integration, and e2e tests for critical paths pass
...
- pre-deploy checklist and rollback path are documented
- smoke test plan is ready

### `post-deploy-verified`
- live smoke tests pass
- health endpoint is green
- no immediate regression in logs/monitoring

If a gate fails, update the relevant task to `blocked` or `failed`, log the reason in `.10x/dev-log.md`, and do not advance automatically.
...
3. Run appropriate verification:
   ...
   - Production: full test suite + build + lint pass, contract checks pass, performance budget reviewed, smoke plan ready
