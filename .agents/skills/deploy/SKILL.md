...
- Rollback procedure with owner and trigger conditions
- Smoke test checklist for post-deploy verification
- Monitoring / alerting hooks or placeholders

## Step 1.5: Release Safety Requirements

For `production`, deployment setup is incomplete until these are documented:
- exact migration order
- feature flags or staged rollout notes, if used
- rollback command/path
- required smoke tests after deploy
- owner for go/no-go decision

If the app changes the database schema, verify the migration plan includes forward safety and rollback notes before shipping.

## Step 2: Generate Config

Based on scope and stack, create the necessary files.

For `production`, prefer adding:
- deployment config
- CI workflow
- health check route or endpoint
- release checklist document or section in `.10x/dev-log.md`
- smoke test script or documented commands when automation is not yet available

## Step 2.5: Post-Deploy Verification

After deployment instructions are generated, define the smoke verification plan:
- health endpoint
- sign-in or auth check if auth exists
- primary revenue or conversion flow
- one read path and one write path for core data
- log/monitoring check

Tell the user exactly what to verify immediately after deploy and what would trigger rollback.
