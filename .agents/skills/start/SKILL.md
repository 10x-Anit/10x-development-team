...
This plugin's index system (`.10x/` files) serves as a cheaper alternative to Codex's `--resume` flag, which has a confirmed cache bug (GitHub #34629) that causes a full cache rebuild on resume. By storing all project context in small JSON files, `/resumeproject` reconstructs context in a fresh session without triggering the bug.

If the user mentions high API costs, suggest:
1. Use `npx @anthropic-ai/Codex` instead of the standalone binary (reported workaround for cache Bug 1 — GitHub #40524)
