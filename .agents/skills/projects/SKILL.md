...
4. If exists: tell user "To continue working on [name], open Codex in [path] and run `/resumeproject`"
   - IMPORTANT: Always recommend `/resumeproject` over `--resume`. The `--resume` flag has a confirmed cache bug ([#34629](https://github.com/anthropics/Codex/issues/34629)) that causes a full cache rebuild on resume. `/resumeproject` reconstructs context from `.10x/` index files in a fresh session, avoiding the bug entirely.
